...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  run(`UPDATE posts SET semana=?,mes=?,dia=?,fecha=?,horario=?,pilar=?,estado=?,objetivo=?,enlace=?,
    formato_imagen=?,formato_carrusel=?,formato_reel=?,formato_historia=?,gancho=?,descripcion=?,
    hashtags=?,indicaciones_diseno=?,notas=?,updated_at=datetime('now') WHERE id=?`,
    [d.semana,d.mes,d.dia,d.fecha,d.horario,d.pilar,d.estado,d.objetivo,d.enlace,
     ...
     d.gancho,d.descripcion,d.hashtags,d.indicaciones_diseno,d.notas, params.id])
