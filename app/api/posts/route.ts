...
export async function POST(req: NextRequest) {
  ...
  const id = insert(
    `INSERT INTO posts (semana,mes,dia,fecha,horario,pilar,estado,objetivo,enlace,
      formato_imagen,formato_carrusel,formato_reel,formato_historia,gancho,descripcion,hashtags,
      indicaciones_diseno,notas,red_social,moodboard_imgs,moodboard_links)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
  ...
  return NextResponse.json(queryAll('SELECT * FROM posts WHERE id = ?', [id])[0], { status: 201 })
}
